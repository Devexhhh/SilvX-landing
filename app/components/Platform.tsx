...
export default function Platform() {
  return (
    <section className="relative w-full bg-brand-bg py-12 md:pt-5 flex flex-col items-center selection:bg-emerald-500/50">
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6">

        {/* Header Section */}
        <div className="mb-5 md:mb-20 text-left">
          {/* Eyebrow text */}
          <div className="flex items-center gap-2 text-[10px] md:text-[11px] font-sans font-bold tracking-[0.2em] uppercase text-brand-silver mb-4 md:mb-6">
          ...
          {/* Responsive Heading */}
          <h2 className="text-3xl md:text-[52px] text-brand-black leading-[1.2] md:leading-[1.1] mb-6 md:mb-8 font-serif tracking-tight max-w-2xl">
            Your Daily Path to an <br className="hidden md:block" />
            Unshakable Silver Legacy
          </h2>

          {/* Subtext */}
          <p className="text-brand-silver font-sans font-light text-base md:text-lg max-w-2xl leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-6">
          {modules.map((m) => (
            <div key={m.title} className="group cursor-pointer">
              ...
              <div
                className={`h-28 md:h-32 rounded-sm mb-5 md:mb-6 flex items-center justify-center relative overflow-hidden transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-brand-black/10 ${m.gradient}`}
              >
                {/* Overlay for depth */}
